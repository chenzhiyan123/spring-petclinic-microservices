...
import org.springframework.samples.analysis.LogExceptionAnalysis;
...
public class LogExceptionRCA extends BusinessCodeRCA {
    @Override
    public void doRCA(InputFault inputFault, FaultRepository faultRepository, FaultEdgeRepository faultEdgeRepository, ExecutorService executorService) {
        ...
        if (fault.isEmpty()) {
            LogExceptionAnalysis analysis = new LogExceptionAnalysis();
            Fault rootFault = analysis.localize(inputFault);
